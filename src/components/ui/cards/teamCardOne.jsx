    ...
    >
      <div className="relative group hover-underline">
        <div className="relative">
          <div className=" absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-500 group-hover:w-full group-hover:h-full group-hover:opacity-100 h-0 w-0 opacity-0 flex justify-center items-center bg-[#D2E0D9CC]">            <ul className="flex items-center gap-7.5">
              {social_link.map(({ id, link, media }, index) => {
                return (
                  <li key={id}>
                    <Link
                      href={link}
                      className={`font-semibold text-lg relative after:contents-[""] after:absolute after:h-[20px] after:w-[1px]  ${
                        index === social_link.length - 1 ? "after:bg-transparent" : "after:bg-black"
